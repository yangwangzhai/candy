...
var GameLayer = cc.Layer.extend({
    ...
    ctor: function () {
        ...
        bg.y = size.height/2;
        /**
        * 利用遮罩可以让指定范围内的内容显示，超出范围的内容隐藏。
         * 步骤；把内容添加到遮罩节点上，再给遮罩节点指定一个裁剪模板，超出裁剪模板的内容将被隐藏。
        **/
        var clippingPanel = new cc.ClippingNode();//新建一个遮罩节点
        this.addChild(clippingPanel, 2);
        this.mapPanel = new cc.Layer(); //新建一个裁剪模板（本质上就是一个有宽，有高的层）
        this.mapPanel.x = (size.width - Constant.CANDY_WIDTH*Constant.MAP_SIZE)/2;  //设定层在X轴上的位置
        this.mapPanel.y = (size.height - Constant.CANDY_WIDTH*Constant.MAP_SIZE)/2; //设定层在Y轴上的位置
        clippingPanel.addChild(this.mapPanel, 1);//遮罩节点添加裁剪模板

        var stencil = new cc.DrawNode();  //设定裁剪模板的宽高
